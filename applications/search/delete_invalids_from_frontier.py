...
from datamodel.search.LeoleJpadill3_datamodel import LeoleJpadill3Link
...
@Deleter(LeoleJpadill3Link)
@Getter(LeoleJpadill3Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(LeoleJpadill3Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(LeoleJpadill3Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_LeoleJpadill3".format(CrawlerFrame.app_id),
